...
class HotelController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        try {
            ...
            if ($request->has('precio') && $request->precio) {
                switch ($request->precio) {
                    case 'economico':
                        ...
                        break;
                    case 'medio':
                        $query->whereBetween('precio', [1000, 1500]);
                        break;
                    case 'alto':
                        $query->where('precio', '>', 1500);
                        break;
                }
            }
            ...
            // Solo hoteles disponibles por defecto
            ...
            $hoteles = $query->paginate($porPagina);
    ...
    public function buscar(Request $request): JsonResponse
    {
        try {
            ...
            $hoteles = Hotel::where('nombre', 'LIKE', "%{$termino}%")
    ...
    {
        try {
            $estados = Hotel::select('estado')
                           ->distinct()
                           ->orderBy('estado')
                           ->pluck('estado');

            $ciudades = Hotel::select('ciudad', 'estado')
                            ->distinct()
                            ->orderBy('estado')
                            ->orderBy('ciudad')
                            ->get()
    ...
    {
        // Implementar si necesitas crear hoteles desde el API
    ...
    {
        // Implementar si necesitas actualizar hoteles desde el API
        // Requeriría validaciones y autorización
    }
    ...
    {
        // Implementar si necesitas eliminar hoteles desde el API
